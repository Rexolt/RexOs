...
static int e1000_send_frame(struct net_device *dev, const void *data, uint32_t len) {
    (void)dev;
    
    /* Bounce buffer: allocating a physical frame because 'data' might be in virt memory */
...
static void e1000_init_tx(void) {
    ...
    e1000_write_reg(E1000_REG_TCTL, (1 << 1) | (1 << 3) | (0x0F << 4) | (0x40 << 12));
}
...
bool e1000_init(void) {
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        if (d->vendor == E1000_VEND && (d->device == E1000_DEV_100E || d->device == E1000_DEV_10D3)) {
            s_e1000_pci = d;
            break;
        }
    }
